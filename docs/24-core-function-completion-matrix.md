...
| CodeGraph Sidecar | CodeGraph 1.5.0 可执行 index/status/explore/impact/affected-tests；查询默认拒绝陈旧索引 | 通过统一 API/MCP 提供只读能力 | 已完成 |
| CodeGraph 对比基线 | 已提交真实 66 节点/70 关系快照及其与平台 72/95 基线的类型、关系和语义映射报告 | 对比为结构化 JSON，可直接消费 | 已完成 |
| 图智能增强 | 独立实现 BM25+确定性语义+RRF、Community、Process 和跨仓 Contract Graph | 返回现有 Graph Explorer 可消费的稳定节点、步骤和证据结构 | 已完成 |
| 只读 MCP Server | 已实现 stdio 与 Streamable HTTP、Origin/Bearer 防护、协议协商、Schema 校验、Token 预算、Resources/Templates/Prompts；不暴露写工具 | Agent 可直接使用 14 个只读工具和仓库级资源 | 已完成 |
